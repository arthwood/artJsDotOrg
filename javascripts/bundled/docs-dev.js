    ...
    main: {
      ...
      sections: [ {
        ...
        members: [ {
          ...
          more: "Some of the class methods from framework can get injected into native classes." + "For example most of com.arthwood.utils.ArrayUtils would then affect native Array object." + 'To see which native classes can be affected by framework see other parts of documentation, especially "utils" package.',
          ...
          more: "<p>Normally after framework is loaded you have two ways of accessing its classes:</p>" + "<ul><li>using full qualified path (com.arthwood.utils.ArrayUtils)</li><li>by looking into top ArtJs namespace (ArtJs.ArrayUtils)</li></ul>" + "<p>For now there is no classes with the same name in whole framework so accessing it via ArtJs is fine." + "However, if at some point it happens, full qualified name is necessary." + "When all framework classes are loaded and accessible (e.g. in js file following the framework file) simply use:</p>",
          example: [ "ArtJs.globalize();" ]
        }, {
          header: "$():Element",
          description: "Alias for ElementUtils.getElements()",
          example: [ "var nav = ArtJs.$('.main p.item span');" ]
        }, {
          header: "$D():Delegate",
          description: "Alias for Delegate.create()",
          example: [ "var delegate = ArtJs.$D(this, this.onClick);" ]
        }, {
          header: "$DC():Function",
          description: "Alias for Delegate.callback()",
          example: [ "var callback = ArtJs.$DC(this, this.onClick, true);" ]
        }, {
          header: "$del():Ajax",
          description: "Alias for Ajax.del()",
          example: [ "var ajax = ArtJs.$del('http://mydomain.com', {id: 4}, new Delegate(this, this.onAjaxSuccess));" ]
        }, {
          header: "$find():Array",
          description: "Alias for ElementUtils.find()",
          example: [ "var nav = ArtJs.$('navigation');", "var items = ArtJs.$down(nav, '.item');" ]
        }, {
          header: "$get():Ajax",
          description: "Alias for Ajax.get()",
          example: [ "var ajax = ArtJs.$get('http://mydomain.com', null, new Delegate(this, this.onAjaxSuccess));" ]
        }, {
          header: "p():Void",
          description: "Debug info. If debug console is accessible output to the console; uses alert() otherwise.",
          example: [ "p('myVariable: ' +  myVariable);" ]
        }, {
          header: "$P():Element",
          description: "Alias for ElementBuilder.parse()",
          example: [ "var element = ArtJs.$P('<span class=\"desc\">Blue t-shirt</span>');" ]
        }, {
          header: "$post():Ajax",
          description: "Alias for Ajax.post()",
          example: [ "var ajax = ArtJs.$post('http://mydomain.com', {id: 4}, new Delegate(this, this.onAjaxSuccess));" ]
        }, {
          header: "$put():Ajax",
          description: "Alias for Ajax.$put()",
          example: [ "var ajax = ArtJs.$put('http://mydomain.com', {id: 4}, new Delegate(this, this.onAjaxSuccess));" ]
        }, {
          header: "$up():Element",
          description: "Alias for ElementUtils.up()",
          example: [ "var nav = ArtJs.$('navigation');", "var parent = ArtJs.$up(nav);" ]
        } ]
      } ]
    },
    list: {
    ...
    component: {
      ...
      sections: [ {
        ...
        members: [ {
          ...
          description: "Registers dependency. Whenever any of dependency classes is instantiated it is passed " + "to onDependency() method which you may need to implement.",
